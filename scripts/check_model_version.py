...
# def check_model_version(env, model_name):
#     print(f"[INFO] env={env}, model_name={model_name}")
...
#         run_id = mv.run_id                               # <-- added

#         print(f"[INFO] Found alias latest-model => version {version}, run_id {run_id}")

#         dbutils.jobs.taskValues.set(key="model_version", value=version)
#         dbutils.jobs.taskValues.set(key="run_id", value=run_id)   # <-- added
#         emit_output(version)
#         return
#     except Exception as e:
#         print(f"[DEBUG] get_model_version_by_alias failed: {e}")

#     # ---- 2) Fallback: search_model_versions ----
#     try:
#         versions = client.search_model_versions(f"name='{model_name}'")
#         if versions:
#             latest_mv = max(versions, key=lambda v: int(v.version))
#             latest = str(latest_mv.version)
#             run_id = latest_mv.run_id                     # <-- added

#             print(f"[INFO] Found latest version via search_model_versions: {latest}, run_id={run_id}")
...
#     # ---- 3) Fallback: get_latest_versions ----
#     try:
...
#             print(f"[INFO] Found latest via get_latest_versions: {latest}, run_id={run_id}")
...
#     dbutils.jobs.taskValues.set(key="run_id", value="UNKNOWN")     # <-- added
#     emit_output("0")
#     return
...
#     env = args.env
#     model_name = args.model_name or os.environ.get("MODEL_NAME") or os.environ.get("model_name")

#     if not model_name:
#         print("[ERROR] model_name missing. Defaulting to model-deploy")
#         model_name = "model-deploy"

#     check_model_version(env, model_name)
